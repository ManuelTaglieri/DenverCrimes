...
public class EventsDao {
		...
		try {
			...
			while(res.next()) {
				try {
					...
					t.printStackTrace();
					System.out.println(res.getInt("id")); //potrebbe evitare il blocco se ci sono date inconsistenti
				}
			}
	...
	public List<Adiacenza> getAdiacenze(String categoria, int mese) {
		String sql = "SELECT e1.offense_type_id as v1, e2.offense_type_id as v2, COUNT(DISTINCT e1.neighborhood_id) AS peso "
				...
				+ "WHERE e1.offense_category_id = ? AND e1.offense_category_id = e2.offense_category_id AND MONTH(e1.reported_date) = ? AND MONTH(e1.reported_date)=MONTH(e2.reported_date) "
